...
export default function UpdateTenantPage() {
  ...
  useEffect(() => {
    ...
    const fetchTenant = async () => {
      try {
        setLoading(true)
        const res = await fetch(`http://localhost:3000/api/admin/tenants/${id}`, { cache: 'no-store' })
        ...
        setForm({
          ...
          type: t.type || 'corporate',
          status: t.status || 'active',
          timezone: t.settings?.timezone || '',
          currency: t.settings?.currency || 'USD',
          language: t.settings?.language || 'en',
          bh_start: t.settings?.businessHours?.start || '',
          bh_end: t.settings?.businessHours?.end || '',
          bh_timezone: t.settings?.businessHours?.timezone || '',
          maxBudget: t.settings?.travelPolicy?.maxBudget ?? '',
          requireApproval: Boolean(t.settings?.travelPolicy?.requireApproval),
          allowedDestinations: t.settings?.travelPolicy?.allowedDestinations || [],
          restrictedDestinations: t.settings?.travelPolicy?.restrictedDestinations || [],
        })
      } catch (e) {
        setErrorMsg('Unable to load tenant')
      } finally {
        setLoading(false)
      }
    }
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:3000/api/admin/tenants/${id}`, {
        method: 'PATCH',
      ...
      router.replace(`/tenants/${id}`)
    } catch (err) {
      setErrorMsg(err.message || 'Unexpected error updating tenant')
  ...
  return (
    <Box sx={{ mt: 4 }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} alignItems="center" justifyContent="space-between" mb={2}>
        <Typography variant="h5" fontWeight={700}>Update Tenant</Typography>
      </Stack>
      <Divider />
